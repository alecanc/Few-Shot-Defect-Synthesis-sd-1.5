"""
dataset_stage2.py - PyTorch Dataset for Stage 2 ([D] token, defect concept)

Stage 2: no prior preservation loss, no DreamBoothDataset combiner. Each item is
just one defect image paired with its conditioning prompt.
...
class Stage2Dataset(Dataset):
    ...
    def __init__(
        ...
        # Fixed prompt for every item, [D] absorbs defect appearance from images
        # not embed defect_type in the prompt
        self.prompt = f"a photo of a {token_D} defect on a surface"

        # Pre-tokenize once
        # every item shares the same prompt, so we tokenize once and store the tensor
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # Select the right transform (handles both mixed and single-type modes)
        if self._mixed_types:
            tfm = (
        ...
        return {
            ...
            "input_ids":    self.input_ids,  # [77] long
            "defect_type":  dtype,           
        }
