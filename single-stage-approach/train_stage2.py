...
def train(cfg: dict, category: str, defect_type: str):
    ...
    dataset = Stage2Dataset(
        ...
        n_clean=stage_cfg.get("n_clean", 8),
    )
                ...
                if global_step % validation_steps == 0:
                    if accelerator.is_main_process:
                        with torch.no_grad():
                            ...
                            # Prompt matches training prompt: [V] and [D] together
                            # shows the product rendered with the defect
                            val_prompt = f"a photo of a {cfg['token_V']} {category} with a {cfg['token_D']} defect"
